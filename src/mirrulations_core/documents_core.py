...
def get_doc_attributes(document_id):
    ...
    """
    logger.debug('Function Successful: % s', 'get_doc_attributes: get_doc_attributes successfully called from '
                                             'process_doc', extra=d)
    if "_" in document_id:
        logger.debug('Calling Function: % s', 'get_doc_attributes: get_doc_attributes calling split', extra=d)
        split_name = re.split('[-_]', document_id)
        logger.debug('Function Successful: % s', 'get_doc_attributes: get_doc_attributes successfully called split',
                     extra=d)
        org = split_name[0] + "_" + split_name[1]
        docket_id = org + '_' + split_name[2]
        document_id = docket_id + "-" + split_name[3]
        logger.debug('Returning: %s', 'get_doc_attributes: returning the organization, docket_id, and document_id',
                     extra=d)
        ...
        logger.debug('Calling Function: % s', 'get_doc_attributes: get_doc_attributes calling add_hyphens', extra=d)
        org = add_hyphens(org_list)
        logger.debug('Function Successful: % s', 'get_doc_attributes: get_doc_attributes successfully called '
                                                 'add_hyphens', extra=d)
        logger.debug('Calling Function: % s', 'get_doc_attributes: get_doc_attributes calling add_hyphens', extra=d)
...
def add_hyphens(list):
    ...
    """
    logger.debug('Function Successful: % s', 'add_hyphens: add_hyphens successfully called from get_doc_attributes',
                 extra=d)
    ...
    logger.debug('Returning: %s', 'add_hyphens: returning the hyphened_string', extra=d)
