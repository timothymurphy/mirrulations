...
def test_main_no_config_setup(ospe):
    with patch('mirrulations_client.__main__.parse_args', return_value={'config': False}) as pa, \
...
def test_main_with_config_setup(ospe):
    with patch('mirrulations_client.__main__.parse_args', return_value={'config': True}) as pa, \
