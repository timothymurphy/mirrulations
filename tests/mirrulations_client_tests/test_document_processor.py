...
GENERIC_DOC_URL = APICallManager('CLIENT').make_document_call_url('DOCUMENT')
...
def test_collect_extra_documents(mock_req, workfile_tempdir):
    ...
    document_url = APICallManager('CLIENT').make_document_call_url(document_id, attachment_number=1, content_type='pdf')

    mock_req.get(GENERIC_DOC_URL, status_code=200, text='{"fileFormats":["' + document_url + '"]}')
    mock_req.get(document_url, status_code=200)
    result = get_extra_documents(APICallManager('CLIENT'), APICallManager('CLIENT').make_call(GENERIC_DOC_URL), workfile_tempdir, document_id)
...
def test_collect_attachments(mock_req, workfile_tempdir):
    ...
    document_url_msw12 = APICallManager('CLIENT').make_document_call_url(document_id, attachment_number=1, content_type='msw12')
    document_url_pdf = APICallManager('CLIENT').make_document_call_url(document_id, attachment_number=1, content_type='pdf')
...
def test_download_document(workfile_tempdir, mock_req):
    document_id = 'FDA-2015-N-0540-0004'
    document_type = 'msw12'
    document_url = APICallManager('CLIENT').make_document_call_url(document_id, attachment_number=1, content_type=document_type)
    mock_req.get(document_url, status_code=200, reason='')
    result = APICallManager('CLIENT').make_document_call(document_id, attachment_number=1, content_type=document_type)
