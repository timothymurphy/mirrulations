...
def test_zip_directory():
    zipped_directory = dz.zip_directory('AHRQ_FRDOC_0001-0036', REGULATIONS_PATH + AHRQ_PATH)
...
def test_zip_directory_bad_id():
    zipped_directory = dz.zip_directory('AHRQ_FRDOC_0001-0037', REGULATIONS_PATH + BAD_AHRQ_PATH)
    archive = zipfile.ZipFile(zipped_directory.filename, 'r')
    readme_file = bytes.decode(archive.read('README.md'))
